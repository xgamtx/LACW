      SUBROUTINE D01DAFM(YA,YB,PHI1,PHI2,F,ABSACC,ANS,NPTS,M,
     .                   P12,KPN1M1,KPN2M2)
C
...
*     Revised by Kepp O.M., 1997. Extra arguments M, P12,KPN1M1,
*     KPN2M2 for sub-integral value calc. added. Use funct. FD.
C     .. Parameters ..
*      CHARACTER*6       SRNAME
*      PARAMETER         (SRNAME='D01DAF')
C     .. Scalar Arguments ..
      DOUBLE PRECISION  ABSACC, ANS, YA, YB, P12,KPN1M1,KPN2M2,
     .                  PHI1, PHI2 ! - more not a functions.
      INTEGER           M, NPTS
...
C     CHECK FOR CONVERGENCE.
      IF (DABS(ANS-EST)-ABSACC) 260, 260, 180
...
C     CALCULATION OF INNER INTEGRAL BEGINS HERE.
  ...
  340 SUMX = (XB+XA)*0.5D0
      DIFFX = (XB-XA)*0.5D0
      FZEROX = F(SUMX,Y,M,P12,KPN1M1,KPN2M2)
  ...
  400 IOLDX = IOLDX + INEWX
      DO 420 JX = INEWX, IOLDX
         IX = IX + 1
         XX = P(IX)*DIFFX
         FUNCTX(JX) = F(SUMX+XX,Y,M,P12,KPN1M1,KPN2M2) +
     .                F(SUMX-XX,Y,M,P12,KPN1M1,KPN2M2)
         IX = IX + 1
         ACUMX = ACUMX + P(IX)*FUNCTX(JX)
  420 CONTINUE
      INEWX = IOLDX + 1
      ...
      IF (DABS(ANSX-ESTX)-ABSACC) 500, 500, 440
  ...
  520 ANSX = 0.0D0
      ...
      END
*
      DOUBLE PRECISION FUNCTION FD(X,Y,M,P12,KPN1M1,KPN2M2)
*     calculate sub-integral value for double integral, used in D01DAFM.
*     (c) 1997, by O.M.Kepp.
      DOUBLE PRECISION X,Y,P12,KPN1M1,KPN2M2,BSJ,TMP,DUMMY
      INTEGER M
      TMP=Y*DSIN(X)		   
      FD=DCOS(Y*DCOS(X)*P12)*Y*TMP
      CALL BESS(M,TMP*KPN1M1,BSJ,DUMMY)
      FD=FD*BSJ
      CALL BESS(M,TMP*KPN2M2,BSJ,DUMMY)
      FD=FD*BSJ
      RETURN
      END
*
      DOUBLE PRECISION FUNCTION FE(X,Y,M,P12,KPN1M1,KPN2M2)
*     calculate sub-integral value for double integral, used in D01DAFM.
*     (c) 1997, by O.M.Kepp.
      DOUBLE PRECISION X,Y,P12,KPN1M1,KPN2M2,BSJ1,TMP,DUMMY
      INTEGER M
      TMP=Y*DSIN(X)
      FE=DCOS(Y*DCOS(X)*P12)*Y*TMP
      CALL BESS(M,TMP*KPN1M1,DUMMY,BSJ1)
      FE=FE*BSJ1
      CALL BESS(M,TMP*KPN2M2,DUMMY,BSJ1)
      FE=FE*BSJ1
      RETURN
      END
*
      DOUBLE PRECISION FUNCTION FF(X,Y,M,P12,KPN1M1,KPN2M2)
*     calculate sub-integral value for double integral, used in D01DAFM.
*     (c) 1997, by O.M.Kepp.
      DOUBLE PRECISION X,Y,P12,KPN1M1,KPN2M2,BSJ,TMP,DUMMY,Eps
      INTEGER M
	DATA Eps/1.D-9/
